...
class _DiscoverPageState extends State<DiscoverPage> {
  ...
  Future<void> _loadAllPois({bool loadMore = false}) async {
    ...
    try {
      final ApiResponse<PoiListData> response = await RetryHelper.apiCall(
        ...
        maxAttempts: 3,
        operationName:
            "Chargement POIs DiscoverPage${loadMore ? ' (load more)' : ''}",
      );

      print(
        '[DISCOVER PAGE] Réponse API reçue - success: ${response.isSuccess}, hasData: ${response.hasData}',
      );
      ...
      if (response.isSuccess && response.hasData) {
        ...
        setState(() {
          if (loadMore) {
            ...
            _categories = _buildCategoriesHierarchy(
  ...
  Future<void> _loadAllPoisForced() async {
    ...
    try {
      final ApiResponse<PoiListData> response = await RetryHelper.apiCall(
        apiRequest: () =>
            _poiService.getPois(perPage: 50, page: 1, useCache: false),
        maxAttempts: 3,
        operationName:
            "Rechargement POIs forcé DiscoverPage (changement langue)",
      );
      ...
      if (response.isSuccess && response.hasData) {
        ...
        if (mounted) {
          ...
          print(
            '[DISCOVER PAGE] POIs rechargés: ${_allPois.length} items (langue: ${_localizationService.currentLanguageCode})',
          );
  ...
  void _applyFilters() {
    ...
    if (_selectedCategory != null) {
      filtered = filtered.where((poi) {
        if (_selectedCategory!.isParentCategory) {
          ...
          final childIds =
              _selectedCategory!.subCategories?.map((sub) => sub.id).toList() ??
              [];

          return poi.categories.any(
            (poiCategory) =>
          ...
          return poi.categories.any(
            (poiCategory) => poiCategory.id == _selectedCategory!.id,
          );
        }
      }).toList();
    }
    ...
    print(
      '[DISCOVER] Filtres appliqués: ${_filteredPois.length}/${_allPois.length} POIs',
    );
  }
  ...
  List<Category> _buildCategoriesHierarchy(List<Category> flatCategories) {
    ...
    for (final category in flatCategories) {
      print(
        '[DISCOVER] Catégorie: ${category.name} (ID: ${category.id}, Level: ${category.level}, ParentId: ${category.parentId})',
      );
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemCount:
            ...
            (_hasMorePages && _filteredPois.length == _allPois.length ? 1 : 0),
